...
const configuredClient = new Promise((resolve, reject) => {
  ...
  return MongoClient.connect(process.env.MONGO_URL).then((client) => {
    ...
    client.oauth_clients = client.collection('oauth_clients');
    client.oauth_access_tokens = client.collection('oauth_access_tokens');
    client.oauth_refresh_tokens = client.collection('oauth_refresh_tokens');
...
export const connection = configuredClient;
